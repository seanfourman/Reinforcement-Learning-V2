...
Two objectives are supported:
  ...
  * ``"cross"`` - a navigate-to-goal gridworld (Cliff-Walking style): each agent
    heads from its spawn to the goal tile, first one there WINS, simultaneous
    arrival is a draw. Slippery junctions add the stochastic transition model.
    THIS is what every shipped round (1-3) uses.
  * ``"race"`` - a legacy key -> gold -> escape contest (colored keys, a stealable
    gold key, teleporter mirrors, lever+trap, ladders). Still implemented below but
    NOT used by any registered round.
...
Coordinates are (row, col); in the shipped cross rounds the theme camera flips the
view, so the goal reads top-centre and the spawns bottom-left/right on screen.
"""
...
class GridWorld(gym.Env):
    ...
    def reset(self, *, seed=None, options=None, regenerate=False):
        if seed is not None:
            ...
            # (previously reset rebuilt the world with the seed but left the slip
            # stochasticity mid-stream)
