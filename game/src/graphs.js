...
function drawVGrid(canvas, grid) {
  ...
  let r0 = H, r1 = -1, c0 = W, c1 = -1, lo = Infinity, hi = -Infinity;
  ...
  const dpr = Math.min(window.devicePixelRatio || 1, 2);
  const cw = canvas.clientWidth || 280, ch = canvas.clientHeight || 280;
  canvas.width = Math.round(cw * dpr); canvas.height = Math.round(ch * dpr);
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  ctx.clearRect(0, 0, cw, ch);
  if (r1 < 0) return;
  const span = (hi - lo) || 1, rows = r1 - r0 + 1, cols = c1 - c0 + 1;
  const tw = cw / cols, th = ch / rows;
  for (let r = r0; r <= r1; r++) for (let c = c0; c <= c1; c++) {
    const v = grid[r][c];
    if (v == null) { ctx.fillStyle = '#e8eaee'; } else {
      const t = (v - lo) / span;
...
function drawVSurface(canvas, grid) {
  ...
  const oy = ch * 0.30;
  const cells = [];
  for (let r = r0; r <= r1; r++) for (let c = c0; c <= c1; c++) if (grid[r][c] != null) cells.push([r, c]);
  cells.sort((a, b) => (a[0] + a[1]) - (b[0] + b[1]));   // back (small r+c) first
  for (const [r, c] of cells) {
    const rr = r - r0, cc = c - c0;
    const t = (grid[r][c] - lo) / span, h = t * barH;
    const x = ox + (cc - rr) * hw, yb = oy + (cc + rr) * hh, yt = yb - h;
    const R8 = 41 + t * 179, G8 = 107 - t * 54, B8 = 235 - t * 189;
    ctx.strokeStyle = 'rgba(20,22,30,0.10)'; ctx.lineWidth = 0.5;
    ctx.fillStyle = shade(R8, G8, B8, 0.72);   // left face
    ctx.beginPath(); ctx.moveTo(x - hw, yt); ctx.lineTo(x, yt + hh); ctx.lineTo(x, yb + hh); ctx.lineTo(x - hw, yb); ctx.closePath(); ctx.fill(); ctx.stroke();
    ctx.fillStyle = shade(R8, G8, B8, 0.55);   // right face
    ctx.beginPath(); ctx.moveTo(x + hw, yt); ctx.lineTo(x, yt + hh); ctx.lineTo(x, yb + hh); ctx.lineTo(x + hw, yb); ctx.closePath(); ctx.fill(); ctx.stroke();
    ctx.fillStyle = `rgb(${Math.round(R8)},${Math.round(G8)},${Math.round(B8)})`;  // top
    ctx.beginPath(); ctx.moveTo(x, yt - hh); ctx.lineTo(x + hw, yt); ctx.lineTo(x, yt + hh); ctx.lineTo(x - hw, yt); ctx.closePath(); ctx.fill(); ctx.stroke();
  }
}
...
export function initDP(parent) {
  parent.insertAdjacentHTML('beforeend', `
    <section id="rl-dp" hidden>
      ...
      <div id="rl-dp-anim" hidden style="margin-top:14px;">
        <div class="ct"><h3>Value propagation (drag to watch V spread from the goal)</h3></div>
  ...
  const chPol = makeChart(q('#rl-ch-dp-pol'), {
    series: [{ key: 'changed', color: '#7c4dd0' }], min: 0, fmt: (v) => v.toFixed(0),
  });
  const polWrap = q('#rl-dp-polwrap');
  window.addEventListener('resize', () => { chDelta.resize(); chMeanV.resize(); chPol.resize(); });
  q('#rl-dp-seg').addEventListener('click', (e) => {
    const b = e.target.closest('button[data-a]'); if (!b) return;
    [...q('#rl-dp-seg').children].forEach((x) => x.classList.toggle('active', x === b));
    dpAgent = b.dataset.a; refresh(); loadSweeps();
  });
  ...
  theta.addEventListener('input', () => {
    ...
    thTimer = setTimeout(() => {
      ...
      setTimeout(() => { refresh(); loadSweeps(); }, 60);
    }, 220);
  });
  paintTheta();
  // ---- Value-Iteration propagation animation (per-sweep V snapshots) ----
  const anim = q('#rl-dp-anim'), vcanvas = q('#rl-dp-vcanvas'), vseek = q('#rl-dp-vseek'), vlbl = q('#rl-dp-vlbl');
  let vframes = [];
  const paintVseek = () => {
  ...
  async function loadSweeps() {
    try {
      const sw = await (await fetch(`/api/dpsweeps?agent=${dpAgent}`, { cache: 'no-store' })).json();
      if (sw.available && sw.frames && sw.frames.length) {
        ...
        renderV();
      } else { anim.hidden = true; vframes = []; }
    } catch (e) { anim.hidden = true; }
  }
  async function refresh() {
    try {
...
export function initBriefing(parent) {
  ...
  async function refresh() {
    try {
      ...
      lastRound = s.round;
      const rewards = (s.rewards || []).map(([k, v]) =>
        `<div class="stat"><span>${k}</span><b>${v > 0 ? '+' : ''}${v}</b></div>`).join('');
      body.innerHTML =
        ...
        (s.slipProb ? `<div class="stat"><span>Slip probability</span><b>${s.slipProb}</b></div>` : '') +
...
export function initCompare(parent) {
  ...
  window.addEventListener('rl-snapshot', (e) => {
    const s = e.detail && e.detail.stats; if (!s) return;
    const rr = s.recentRate || {}, ret = s.lastReturn || {}, q = s.qStates || {}, sc = s.score || {}, rstd = s.returnStd || {};
    const pct = (x) => (100 * (x || 0)).toFixed(0) + '%';
    el.innerHTML =
      cmpHead(s.round ? s.round.labelRed : 'Red', s.round ? s.round.labelBlue : 'Blue') +
      cmpRow('Round win rate', pct(rr.red), pct(rr.blue), lead(rr.red || 0, rr.blue || 0)) +
      cmpRow('Tournament', sc.red || 0, sc.blue || 0, lead(sc.red || 0, sc.blue || 0)) +
      cmpRow('Avg ep length', s.avgEpisodeLen ?? '-', s.avgEpisodeLen ?? '-', '') +
      cmpRow('Last return', (ret.red || 0).toFixed(2), (ret.blue || 0).toFixed(2), lead(ret.red || 0, ret.blue || 0)) +
      cmpRow('Return σ (variance)', (rstd.red || 0).toFixed(2), (rstd.blue || 0).toFixed(2), '') +
      cmpRow('Learned states', (q.red || 0).toLocaleString(), (q.blue || 0).toLocaleString(), '');
  });
}
...
export function initExplore(parent) {
  ...
  async function refresh() {
    try {
      ...
      if (!bb.floor) { sec.hidden = true; return; } // arena round has no cells
      sec.hidden = false;
      body.innerHTML =
        cmpHead('Red', 'Blue') +
        cmpRow('Board coverage', (100 * rb.coverage).toFixed(0) + '%', (100 * bb.coverage).toFixed(0) + '%', lead(rb.coverage, bb.coverage)) +
        cmpRow('Cells visited', `${rb.unique}/${rb.floor}`, `${bb.unique}/${bb.floor}`, '') +
        cmpRow('Visit entropy', rb.entropy.toFixed(2), bb.entropy.toFixed(2), lead(rb.entropy, bb.entropy)) +
        cmpRow('Most-stepped', rb.maxVisits.toLocaleString(), bb.maxVisits.toLocaleString(), '');
    } catch (e) { /* warming up */ }
  }
...
export function initDueling(parent) {
  ...
  async function refresh() {
    try {
      const d = await (await fetch('/api/va?agent=blue', { cache: 'no-store' })).json();
      if (!d.available) { sec.hidden = true; return; }  // only the Dueling side
      sec.hidden = false;
      vEl.textContent = d.v.toFixed(3);
      const a = d.a || [];
      const mx = Math.max(0.01, ...a.map((x) => Math.abs(x)));
      body.innerHTML = a.map((av, i) => {
        const w = (100 * Math.abs(av) / mx).toFixed(0);
        return `<div class="actrow"><span class="al">${i === 8 ? '∅' : i}</span>` +
...
export function initReward(parent) {
  ...
  const PARTS = [['Terminal', 'terminal', '#37b26a'], ['Shaping', 'shape', '#7c4dd0'], ['Other', 'other', '#8a8d94']];
  const rows = (s, color) => {
    const tot = Math.max(0.01, Math.abs(s.terminal) + Math.abs(s.shape) + Math.abs(s.other));
    return PARTS.map(([label, key, c]) => {
      const v = s[key], w = (100 * Math.abs(v) / tot).toFixed(0);
      return `<div class="cmp-row"><span class="cl">${label}</span>` +
  ...
  async function refresh() {
    try {
      const d = await (await fetch('/api/reward', { cache: 'no-store' })).json();
      if (!d.available) { sec.hidden = true; return; }
      sec.hidden = false;
      body.innerHTML =
        `<div class="brief-sub" style="color:#e60012">Red · ${d.episodes}-ep avg</div>` + rows(d.red, '#e60012') +
        `<div class="brief-sub" style="color:#1f5fd0;margin-top:12px">Blue</div>` + rows(d.blue, '#1f5fd0');
    } catch (e) { /* warming up */ }
  }
  setInterval(refresh, 2500);
  refresh();
...
export function initProbe(parent) {
  ...
  const ch = makeChart(parent.querySelector('#rl-ch-probe'), {
    series: [{ key: 'redV', color: '#e60012' }, { key: 'blueV', color: '#1f5fd0' }], fmt: (v) => v.toFixed(2),
  });
  window.addEventListener('resize', () => ch.resize());
  async function refresh() {
    try {
      const d = await (await fetch('/api/qprobe', { cache: 'no-store' })).json();
      if (!d.available || !d.points.length) { sec.hidden = true; return; }
      sec.hidden = false;
      ch.draw(d.points);
    } catch (e) { /* warming up */ }
  }
...
export function initPolicyDiff(parent) {
  ...
  async function refresh() {
    try {
      const d = await (await fetch('/api/polagree', { cache: 'no-store' })).json();
      if (!d.available) { sec.hidden = true; return; }
      sec.hidden = false;
...
function drawPath(canvas, path, color) {
  const ctx = canvas.getContext('2d');
  const dpr = Math.min(window.devicePixelRatio || 1, 2);
  const cw = canvas.clientWidth || 140, ch = canvas.clientHeight || 140;
  canvas.width = Math.round(cw * dpr); canvas.height = Math.round(ch * dpr);
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  ctx.clearRect(0, 0, cw, ch);
  if (!path || path.length < 2) return;
  let r0 = Infinity, r1 = -Infinity, c0 = Infinity, c1 = -Infinity;
  for (const [r, c] of path) { if (r < r0) r0 = r; if (r > r1) r1 = r; if (c < c0) c0 = c; if (c > c1) c1 = c; }
  r0 -= 1; r1 += 1; c0 -= 1; c1 += 1;
  ...
  ctx.beginPath();
  path.forEach(([r, c], i) => { const x = X(c), y = Y(r); i ? ctx.lineTo(x, y) : ctx.moveTo(x, y); });
  ctx.stroke();
  const st = path[0], en = path[path.length - 1];
  ctx.fillStyle = '#8a8d94'; ctx.beginPath(); ctx.arc(X(st[1]), Y(st[0]), 3.5, 0, 7); ctx.fill();
  ctx.fillStyle = color; ctx.beginPath(); ctx.arc(X(en[1]), Y(en[0]), 4.5, 0, 7); ctx.fill();
}
...
export function initTrajectories(parent) {
  ...
  const cr = parent.querySelector('#rl-traj-r'), cb = parent.querySelector('#rl-traj-b');
  async function loadPath(agent) {
    try {
      const r = await (await fetch(`/api/replay?which=top&agent=${agent}&rank=0`, { cache: 'no-store' })).json();
      if (!r.available || !r.frames) return null;
      return r.frames.map((f) => f[agent]).filter((p) => Array.isArray(p) && p.length >= 2);
    } catch (e) { return null; }
  }
  async function refresh() {
    const [pr, pb] = await Promise.all([loadPath('red'), loadPath('blue')]);
    if ((!pr || !pr.length) && (!pb || !pb.length)) { sec.hidden = true; return; }
    sec.hidden = false;
